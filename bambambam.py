...
getFileNamesFromFolder = lambda path, extension : sum([[os.path.join(root,file) for file in files if file[-(1+len(extension)):]=="."+extension] for (root, dirs, files) in os.walk(path)],[])
